<template>
  <main class="folders-page">
    ...
    <section class="folders-body">
      <div class="folders-toolbar"><div><span class="eyebrow">Your blocks</span><strong>{{ folders.length }} {{ folders.length === 1 ? 'folder' : 'folders' }}</strong></div><router-link to="/plans" class="plans-link">Browse training plans →</router-link></div>
      ...
      <div v-else class="folder-grid">
        <article v-for="folder in folders" :key="folder.id" class="folder-card">
          ...
          <router-link :to="`/training-folders/${folder.id}`" class="folder-card-link"><h2>{{ folder.name }}</h2></router-link><p>{{ folder.description || 'A focused training block.' }}</p>
...
<style scoped>
.folders-page{min-height:100vh;padding-top:var(--nav-h,66px);background:#fbf6ec;color:#16130f}.folders-hero{padding:72px max(24px,calc((100% - 1100px)/2));background:#2a55f5;color:#fff}.folders-hero h1{font-size:clamp(42px,7vw,76px);margin:8px 0}.folders-hero p:last-child{margin:0;opacity:.86}.eyebrow{font:11px/1 'Spline Sans Mono',monospace;text-transform:uppercase;letter-spacing:.14em}.folders-body{max-width:1100px;margin:auto;padding:36px 24px}.folders-toolbar{display:flex;justify-content:space-between;align-items:end;gap:16px;margin-bottom:18px}.folders-toolbar strong{display:block;margin-top:6px;font-size:1.2rem}.plans-link{color:#2a55f5;font:11px 'Spline Sans Mono',monospace;font-weight:700;text-transform:uppercase;text-decoration:none}.plans-link:hover{text-decoration:underline}.folder-create{display:grid;grid-template-columns:1.2fr 1.6fr 180px auto;gap:10px;margin-bottom:28px}.folder-create input{border:2px solid #16130f;background:#fff;padding:14px;font:14px inherit}.folder-create button{border:2px solid #16130f;background:#f5d547;padding:0 18px;font-weight:700}.folder-create button:disabled{opacity:.6;cursor:wait}.folder-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:16px}.folder-card{border:2px solid #16130f;background:#fff;padding:22px;min-height:210px;box-shadow:5px 5px 0 #16130f}.folder-card-top{display:flex;justify-content:space-between;align-items:center}.folder-mark{display:grid;place-items:center;width:42px;height:42px;border:2px solid #16130f;background:#f5d547;font-weight:800;font-size:20px}.delete-btn{border:0;background:none;text-decoration:underline;font-size:12px}.folder-card-link{color:#16130f;text-decoration:none}.folder-card-link:hover{color:#2a55f5}.folder-card h2{margin:22px 0 8px;font-size:24px}.folder-card p{color:#665f55;min-height:40px}.folder-meta{display:flex;justify-content:space-between;border-top:1px solid #ddd4c5;padding-top:12px;font:11px 'Spline Sans Mono',monospace;text-transform:uppercase}.folder-items{display:flex;flex-wrap:wrap;gap:6px;margin-top:14px}.folder-items span{background:#e6edff;padding:5px 7px;font-size:11px}.folder-empty{display:flex;flex-direction:column;gap:8px;padding:60px 20px;border:2px dashed #aaa;text-align:center;color:#665f55}.folder-error{color:#b42318}@media(max-width:800px){.folders-toolbar{align-items:flex-start;flex-direction:column;gap:8px}.folder-create{grid-template-columns:1fr}.folder-create button{padding:14px}.folder-grid{grid-template-columns:1fr}}
</style>
