...
const integrationCards = computed(() => integrationStatuses.value.map(status => ({
  ...
  icon: ({ whoop: 'bi bi-heart-pulse', oura: 'bi bi-circle', coros: 'bi bi-watch', garmin: 'bi bi-stopwatch', strava: 'bi bi-bicycle', 'apple-health': 'bi bi-heart' }[status.provider] || 'bi bi-link-45deg'),
...
onMounted(async () => {
  ...
  const providers = ['whoop', 'oura', 'coros', 'garmin', 'strava', 'apple-health']
